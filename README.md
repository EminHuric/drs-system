...
2. [vercel.com](https://vercel.com) → **Add New Project** → uvezite repo.
   Vercel sam prepozna Vite. **Root Directory ostavite prazno** (`./`) — projekat
   je u korenu repozitorijuma, nije u podfolderu.
3. **Environment Variables** → prepišite svih 7 redova iz `.env`
   (6 Firebase vrednosti + `VITE_SUPER_ADMIN_EMAILS`).
4. **Deploy.**

> ⚠️ Ako ste ranije podesili **Root Directory** na neki podfolder, obrišite tu
> vrednost — inače dobijate `vite: command not found`, jer Vercel traži projekat
> tamo gde ga nema.
...
| `/panel` daje 404 na Vercel-u | `vercel.json` mora biti u korenu repozitorijuma. |
| `vite: command not found` na Vercel-u | Root Directory u Vercel podešavanjima nije koren repozitorijuma. Postavite ga na `./` i pustite novi deploy. |
